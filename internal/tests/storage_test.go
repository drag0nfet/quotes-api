...
func TestGetAllQuotesStorage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			store = storage.NewMemoryStorage()
			for idx, quote := range tt.addQuotes {
				store.AddQuote(quote)
				tt.addQuotes[idx].ID = idx + 1
			}
			...
			sort.Slice(tt.addQuotes, func(i, j int) bool {
				return tt.addQuotes[i].ID < tt.addQuotes[j].ID
			})

			if len(quotes) != len(tt.addQuotes) {
				t.Errorf("Expected %d quotes, got %d", len(tt.addQuotes), len(quotes))
			}

			for i := range quotes {
				if quotes[i] != tt.addQuotes[i] {
					t.Errorf("Mismatch at index %d: got %+v, want %+v", i, quotes[i], tt.addQuotes[i])
				}
			}
		})
	}
